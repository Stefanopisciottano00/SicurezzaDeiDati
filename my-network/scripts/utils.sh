...
function printHelp() {
  ...
  if [ "$USAGE" == "prereq" ]; then
    ...
    println "    -c <channel name> - Name of channel to create (defaults to \"hospitalchannel\")"
    ...
    println
    println " Examples:"
    println "   network.sh up createChannel -ca -c hospitalchannel -s couchdb "
    println "   network.sh up createChannel -bft -c hospitalchannel -s couchdb "
