...
You should be able to see the contract starting in the monitoring window. There will be two containers running, one for hospital1 and one for hospital2. The container names contain the organization/peer and the name of the chaincode.
...
```bash
# Environment variables for hospital1

export CORE_PEER_TLS_ENABLED=true
export CORE_PEER_LOCALMSPID="hospital1MSP"
export CORE_PEER_TLS_ROOTCERT_FILE=${PWD}/organizations/peerOrganizations/hospital1.example.com/tlsca/tlsca.hospital1.example.com-cert.pem
...
# invoke the function
peer chaincode query -C hospitalchannel -n basicts -c '{"Args":["org.hyperledger.fabric:GetMetadata"]}' | jq
```

If you don't have `jq` installed omit `| jq`.  The metadata shows the details of the deployed contract and is JSON, so jq makes it easier to read.  You can repeat the above commands for hospital2 to confirm that is working.
...
```bash
    docker run --rm -d --name peer0hospital1_assettx_ccaas  \
                  --network fabric_test \
...
In the peer's environment configuration we then set for hospital1's peer1

```bash
CHAINCODE_AS_A_SERVICE_BUILDER_CONFIG="{\"peername\":\"hospital1peer1\"}"
```

The external builder will then resolve this address to be `hospital1peer1_assettransfer_ccaas:9999` for the peer to use.
