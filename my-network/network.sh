...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cfssl" ]; then
    ...
    peer_cert peer peer0.hospital1.example.com hospital1
    peer_cert admin Admin@hospital1.example.com hospital1

    infoln "Creating hospital2 Identities"
    #function_name cert-type   CN   org
    peer_cert peer peer0.hospital2.example.com hospital2
    peer_cert admin Admin@hospital2.example.com hospital2
...
function networkDown() {
  ...
  COMPOSE_ORG3_BASE_FILES="-f addHospital3/compose/${COMPOSE_FILE_ORG3_BASE} -f addHospital3/compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_ORG3_BASE}"
  COMPOSE_ORG3_COUCH_FILES="-f addHospital3/compose/${COMPOSE_FILE_ORG3_COUCH} -f addHospital3/compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_ORG3_COUCH}"
  COMPOSE_ORG3_CA_FILES="-f addHospital3/compose/${COMPOSE_FILE_ORG3_CA} -f addHospital3/compose/${CONTAINER_CLI}/${CONTAINER_CLI}-${COMPOSE_FILE_ORG3_CA}"
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.example.com docker_peer0.hospital1.example.com docker_peer0.hospital2.example.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf addHospital3/fabric-ca/hospital3/msp addHospital3/fabric-ca/hospital3/tls-cert.pem addHospital3/fabric-ca/hospital3/ca-cert.pem addHospital3/fabric-ca/hospital3/IssuerPublicKey addHospital3/fabric-ca/hospital3/IssuerRevocationPublicKey addHospital3/fabric-ca/hospital3/fabric-ca-server.db'
...
COMPOSE_FILE_ORG3_BASE=compose-hospital3.yaml
# use this as the docker compose couch file for hospital3
COMPOSE_FILE_ORG3_COUCH=compose-couch-hospital3.yaml
# certificate authorities compose file
COMPOSE_FILE_ORG3_CA=compose-ca-hospital3.yaml
#
